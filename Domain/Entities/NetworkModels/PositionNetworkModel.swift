...
extension PositionModel {
	...
	extension PositionNetworkModel.Response {
		public func asDomain() -> PositionModel? {
			...
			if let open = self.isOpen, open {
				var side: OrderSide?
				if let safeQuantity = self.currentQty {
					side = (safeQuantity < 0) ? .short : .long
				} else if let safeLiquid = self.liquidationPrice, let safeEntry = self.avgEntryPrice {
					side = (safeLiquid < safeEntry) ? .long : .short
				}else {
				return nil
				}

			return PositionModel(symbol: self.symbol, quantity: self.currentQty ?? 0, profitPercent: self.unrealisedPnl ?? 0, liquid: self.liquidationPrice ?? 0, entry: self.avgEntryPrice ?? 0, leverage: self.leverage ?? 1, side: side! )
			}
